...
{
	private static final String OTHER_HOME_PERMISSION = SpawnControl.BASE_PERMISSION_NODE + ".home.others";
	...
	public boolean execute(Player p, org.bukkit.command.Command command, String[] args) {
		...
		if( args.length > 0 && plugin.hasPermission(p, OTHER_HOME_PERMISSION) ) {
			...
			com.aranai.spawncontrol.entity.Home home = util.getHome(args[0], world);
			...
			if( home != null ) {
				p.sendMessage("Teleporting to player home for "+home.getPlayerName()+" on world "+world);
