/** This file borrowed from LWC for the purposes of this project.
 *  Original license info included below.
 * 
 * ************************************************************
 * 
 ...
 *
 */
public class ConfigYAMLNode {
    protected Map<String, Object> root;

    public ConfigYAMLNode(Map<String, Object> root) {
        this.root = root;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public List<ConfigYAMLNode> getNodeList(String path, List<ConfigYAMLNode> def) {
        List<Object> raw = getList(path);
        if (raw == null) {
            return def != null ? def : new ArrayList<ConfigYAMLNode>();
        }

        List<ConfigYAMLNode> list = new ArrayList<ConfigYAMLNode>();
        for (Object o : raw) {
            if (o instanceof Map) {
                list.add(new ConfigYAMLNode((Map<String, Object>) o));
            }
        }
     ...
     */
    @SuppressWarnings("unchecked")
    public ConfigYAMLNode getNode(String path) {
        Object raw = getProperty(path);
        if (raw instanceof Map) {
            return new ConfigYAMLNode((Map<String, Object>) raw);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    public Map<String, ConfigYAMLNode> getNodes(String path) {
        Object o = getProperty(path);
        if (o == null) {
            return null;
        } else if (o instanceof Map) {
            Map<String, ConfigYAMLNode> nodes =
                    new HashMap<String, ConfigYAMLNode>();

            for (Map.Entry<String, Object> entry : ((Map<String, Object>) o).entrySet()) {
                if (entry.getValue() instanceof Map) {
                    nodes.put(entry.getKey(),
                            new ConfigYAMLNode((Map<String, Object>) entry.getValue()));
                }
            }
